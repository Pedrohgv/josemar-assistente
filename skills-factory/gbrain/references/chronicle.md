...
Auto-emission (on every new `put_page` of an eligible page) requires `auto_chronicle=true` (operator config, requires `--force` on v0.42.57.0 due to a config-key registry bug).
