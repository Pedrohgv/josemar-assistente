...
| `JOSEMAR_MCP_AUTHORIZED_KEY` | No | Required when `JOSEMAR_MCP_ENABLED=true`; single-line SSH public key for the forced-command MCP endpoint |
...
| `JOSEMAR_MCP_ENABLED` | No | Enable the optional `josemar-mcp` overlay (default `false`). See `docs/josemar-mcp.md`. Requires `HERMES_API_SERVER_ENABLED=true` and `HERMES_API_SERVER_KEY` when enabled. |
| `JOSEMAR_MCP_SUBNET` | No | Override the internal `josemar-mcp` Docker subnet (default `172.31.251.0/29`, distinct from `browser-control`). Set together with the gateway/IP overrides if the default collides. |
| `JOSEMAR_MCP_GATEWAY` | No | Override the `josemar-mcp` gateway IPv4 (default `172.31.251.1`). |
| `JOSEMAR_MCP_HERMES_IP` | No | Override Hermes's static IPv4 on `josemar-mcp` (default `172.31.251.2`); the sshd (running inside the hermes container) binds only to this IP and Tailscale Serve forwards to it. |
...
- Josemar MCP is an optional Compose overlay (`docker-compose.josemar-mcp.yml`), disabled by default (`JOSEMAR_MCP_ENABLED=false`). When enabled, cont-init script `02-josemar-mcp-sshd` provisions a hardened sshd that the s6 longrun `/etc/services.d/josemar-mcp-sshd/run` supervises INSIDE the existing Hermes container, sharing the same PID namespace, gbrain PGLite state, and locks as the gateway. The sshd runs foregrounded as root under s6 and drops to the `hermes` user for forced commands; the gateway continues as non-root `hermes` via `s6-setuidgid`. Deploy validates `JOSEMAR_MCP_AUTHORIZED_KEY` is a single-line SSH public key, requires `HERMES_API_SERVER_ENABLED=true` and `HERMES_API_SERVER_KEY` (the `josemar_chat` tool calls the internal Hermes API using that key), populates the `josemar-mcp-authorized-keys` named volume atomically, writes the combined Tailscale Serve config (tcp:2223 alone, or tcp:2222 + tcp:2223 when browser-control is also enabled — no Funnel), sets `COMPOSE_FILE` to include `docker-compose.josemar-mcp.yml` (ordered after browser-control and before embeddings), and verifies the sshd process is running inside hermes, the gateway is still non-root, Tailscale Serve tcp:2223 targets the exact Hermes IP with no Funnel, and the forced-command wrapper + MCP server are present and import cleanly. When disabled, deploy clears `josemar-mcp-authorized-keys`, writes the Serve config without tcp:2223 (or `{}` if browser-control is also disabled), and verifies tcp:2223 is absent and no josemar-mcp sshd process is running. The `josemar-mcp` network uses a distinct subnet (`172.31.251.0/29`) from `browser-control` (`172.31.250.0/29`) so both overlays can be enabled simultaneously. SSH user (`hermes`), SSH port (`2223`), and the forced command are fixed constants. No separate sidecar container is used.
