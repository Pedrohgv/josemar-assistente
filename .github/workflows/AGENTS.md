...
| `TASKNOTES_DAILY_LINKS_ENABLED` | No | Strict master switch (default `true`; a missing/empty repository variable resolves to `true`) for the TaskNotes MCP daily-note task-link projection and its reconciliation. A nonempty value must be case-insensitive `true`/`false` (normalized to lowercase in `.env`); anything else is rejected before any deploy mutation and again at MCP engine init. `false` disables the projection and makes reconciliation fully inert. No Daily Notes folder/date/template or vault path is env-configurable. |
| `TASKNOTES_DAILY_LINKS_RECONCILE_ENABLED` | No | Strict slave switch (default `true`) gating the TaskNotes MCP's Daily Note link reconciliation in BOTH lanes: the pre-mutation reconciliation and the refresh-cycle (`josemar-gbrain refresh`) reconciliation. Same validation rules; effective only while `TASKNOTES_DAILY_LINKS_ENABLED` is also `true`; with either flag off the gated lanes are inert. |
