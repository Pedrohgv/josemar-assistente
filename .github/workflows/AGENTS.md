...
| `TASKNOTES_DAILY_LINKS_ENABLED` | No | Strict default-off switch (missing/empty = `false`) for TaskNotes MCP daily-note task-link projection. A nonempty value must be case-insensitive `true`/`false` (normalized to lowercase in `.env`); anything else is rejected before any deploy mutation and again at MCP engine init. No Daily Notes folder/date/template or vault path is env-configurable. |
