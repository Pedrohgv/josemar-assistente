#!/usr/bin/env python3
"""Lightweight documentation integrity checks for Josemar.

This intentionally avoids a full Markdown toolchain. It checks repository-local
references that coding workers depend on and flags obvious always-loaded context
regressions without turning heuristic size guidance into a strict style system.
"""
...
def check_skill_references(root: Path) -> list[Finding]:
    ...
    for skill_file in sorted(skills_root.glob("*/SKILL.md")):
        ...
        for referenced_skill, relative_path in SKILL_VIEW_RE.findall(text):
            target = skills_root / referenced_skill / relative_path
            if not target.is_file():
                errors.append(
                    Finding(
                        skill_file,
    ...
    for skill_file in sorted((root / "skills-factory").glob("*/SKILL.md")):
        ...
        if lines > SKILL_ERROR_LINES:
            errors.append(
                Finding(
                    skill_file,
                    f"{lines} lines exceeds the {SKILL_ERROR_LINES}-line guardrail; keep routine use self-contained but move non-routine depth to references/",
                )
            )
            ...
            warnings.append(
                Finding(
                    skill_file,
                    f"{lines} lines exceeds the {SKILL_WARN_LINES}-line review heuristic; confirm the extra context is routine-use material",
                )
            )
    ...
    for agents_file in (path for path in documentation_files(root) if path.name == "AGENTS.md"):
        ...
        if lines > AGENTS_WARN_LINES:
            warnings.append(
                Finding(
                    agents_file,
                    f"{lines} lines exceeds the {AGENTS_WARN_LINES}-line review heuristic; consider narrower routing/on-demand docs",
                )
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for warning in warnings:
        print(warning.render(args.root.resolve(), "WARNING"), file=sys.stderr)
    for error in errors:
        print(error.render(args.root.resolve(), "ERROR"), file=sys.stderr)
